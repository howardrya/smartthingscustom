...
metadata {
	definition (name: "NodeMCU Garage Door Control - Right", namespace: "smartthings", author: "Ryan") {
		capability "Actuator"
...
tiles(scale: 2) {
        ...
        standardTile("openright", "device.Button", width: 2, height: 2, decoration: "flat") {
			state "iconLabel", label: 'Right Door', action: "openright", icon:"st.doors.garage.garage-closed", backgroundColor: "#ffffff", nextState: "refreshing"
            state "refreshing", label: 'Sending', action: "ResetTiles", icon: "st.secondary.refresh-icon", backgroundColor: "#44b621", nextState: "iconLabel"
		}
		...
		main "openright"
		details(["status","openright", "RefreshTrigger", "RebootNow"])
	}
}
...
//	log.debug "Parsing '${description}'"
	...
	if (descMap["body"]) {
		if (headersReturned.contains("application/json")) {
			...
			if (line.contains('Contact Sensor 2=Open')) { jsonlist.put ("SensorPinStatus".replace("=",""), "Open") }
			if (line.contains('Contact Sensor 2=Closed')) { jsonlist.put ("SensorPinStatus".replace("=",""), "Closed") }
		}
	}
}
